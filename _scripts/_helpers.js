module.exports = {
  /**
   * Check if nuxtpress containers are running
   */
  ...
  checkWPCli: function(container, done) {
    var exec = require("child_process").exec;
    exec("docker exec " + container + " bash -c 'wp'", function(
      error,
      stdout,
      stderr
  ...
  installWPCli: function(container, done) {
    ...
    exec(
      "docker exec " +
        container +
        " bash -c 'apt-get update && apt-get install -y less && curl -O https://raw.githubusercontent.com/wp-cli/builds/gh-pages/phar/wp-cli.phar && chmod +x wp-cli.phar && mv wp-cli.phar /usr/local/bin/wp && wp --allow-root cli'",
  ...
  dockerUp: function(done) {
    ...
    exec("cat .env", function(error, stdout, stderr) {
      ...
      exec("docker-compose up --force-recreate -d", function(
        error,
        stdout,
        stderr
      ) {
  ...
  runYarn: function(container, done) {
    ...
    var exec = require("child_process").exec;
    exec(
      "docker exec " + container + " bash -c 'yarn " + command.join(" ") + "'",
      function(error, stdout, stderr) {
        console.log(stdout);
      }
    ).on("exit", function(code) {
    ...
    this.getArgs(
      ...
      function(err, result) {
        ...
        exec(
          "docker exec " +
            container +
            " bash -c 'wp --allow-root scaffold post-type " +
            slug +
            " --label=" +
            result.name +
            " --textdomain=nuxtpress'",
          function(error, stdout, stderr) {
            if (!error) {
              mkdirp(
                "./wp-content/themes/nuxtpress/custom-post-types",
                function(err) {
                  if (err) {
                    console.log(
                      "ERROR: Could not create post-type (" + err + ")."
                    );
                    return;
                  }
                  ...
                  fs.writeFile(
                    ...
                    function(err) {
                      ...
                      console.log(
                        "SUCCESS: Post type " + result.name + " created."
                      );
                    }
                  );
                }
    ...
    this.getArgs(
      [
        ...
        },
        {
          name: "cpts",
          description:
            "Enter a commaseparated list of post types for the taxonomy, e.g. event,presentation",
          type: "string",
          pattern: /^\w+(,\w+)*$/,
      ...
      function(err, result) {
        ...
        exec(
          "docker exec " +
            container +
            " bash -c 'wp --allow-root scaffold taxonomy " +
            slug +
            " --label=" +
            result.name +
            " --post_types=" +
          ...
          function(error, stdout, stderr) {
            if (!error) {
              mkdirp("./wp-content/themes/nuxtpress/taxonomies", function(err) {
                if (err) {
                  console.log(
                    "ERROR: Could not create taxonomy (" + err + ")."
                  );
                  return;
                }

                fs.writeFile(
                  "./wp-content/themes/nuxtpress/taxonomies/" + slug + ".php",
                  stdout,
                  function(err) {
                    if (err) {
                      console.log(
                    ...
                    console.log(
                      "SUCCESS: Taxonomy " + result.name + " created."
                    );
                  }
                );
              });
