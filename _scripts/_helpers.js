module.exports = {
  ...
  generateWPTax: function(container, done) {
    ...
    this.getArgs([{
      ...
      exec('docker exec ' + container + ' bash -c \'wp --allow-root scaffold taxonomy ' + slug + ' --label=' + result.name + ' --post_types=' + result.cpts + ' --textdomain=nuxtpress\'', function(error, stdout, stderr) {
        if (!error) {
          ...
          mkdirp('./wp-content/themes/nuxtpress/taxonomies', function (err) {
            ...
            fs.writeFile('./wp-content/themes/nuxtpress/taxonomies/' + slug + '.php', stdout, function(err) {
              if (err) {
                console.log('ERROR: Could not create taxonomy (' + err + ').')
                return;
              }
