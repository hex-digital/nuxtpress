...
**NuxtPress** combines **_WordPress_**, the worlds biggest CMS with **_nuxt.js_**,
a brilliant front-end application framework yet.
...
The goal of NuxtPress is to provide a ready to use development environment, which makes the
...
All together the **NuxtPress** features get you started with your front-end with just
...
-   **_MySql_** (`mysql.nuxtpress`) Database for your **_WordPress_** installation. The data-folder
    of the database-container is mirrored to the \_db-folder of your host system, to
    keep the data persistent.

-   **_WordPress_** (`wp.nuxtpress`) on a **_Apache_** server with the newest **_PHP_** version and
    the **NuxtPress** Rest API extension theme, **_ACF_** and other good-to-have plugins
    pre-installed. The wp-content directory of the **_WordPress_** directory is
    mirrored to the wp-content directory on your host.

-   **_nuxt.js_** (`front.nuxtpress`) started in development mode with file-monitoring and
...
**nuxtpress** allows you to change the above setup to run multiple projects at the
...
The script creates an `.env` file like the folllowing and stops the old containers:
    ...
    NUXTPRESS_NUXT_CONTAINER=front.projectname
...
**nuxtpress** theme to activate all the API extensions. Additionally you might want
...
that the **nuxtpress** info screen is showing.
...
be greeted by the **NuxtPress** intro-screen.
...
To make a complete deploy of NUXTPRESS, you have to get at least one server and solve
...
action, so while generating, your NUXTPRESS site will be down and started again
after.
...
To get the files you go to the `dist` directory inside your `nuxtpress/nuxt` directory:
...
**_WordPress_** menus. You can view that list with the following endpoint:
    ...
    GET: /wp-json/nuxtpress/v1/generate
...
```
GET: /wp-json/nuxtpress/v1/front-page
GET: /wp-json/nuxtpress/v1/front-page?_embed
```
...
```
GET: /wp-json/nuxtpress/v1/menu
GET: /wp-json/nuxtpress/v1/menu?location=<location>
```
...
```
GET: /wp-json/nuxtpress/v1/slug/<post-or-page-slug>
GET: /wp-json/nuxtpress/v1/slug/<post-or-page-slug>?_embed
```
...
To help you with some of the common tasks in **nuxtpress**, we integrated a bunch of
...
enter the **NUXTPRESS** containers, you can use the following ***npm***
scripts:
...
**NUXTPRESS** provides you with the full power of the
***WP-CLI*** tool. Check out all documentation at [https://developer.wordpress.org/cli/commands/](https://developer.wordpress.org/cli/commands/). To run any ***WP-CLI*** command inside the `wp.nuxtpress`
container, just use the following ***npm***-script:

    npm run wp <wp-cli-command>

Examples: `npm run wp plugin list`, `npm run wp plugin install advanced-custom-fields`, `npm run wp user create nuxtpress me@nuxtpress.io`
...
**NUXTPRESS** allows you to generate custom post types and taxonomies via ***npm***
...
The custom post type definition is copied into the `cpts` folder of the nuxtpress
theme and loaded automatically by the theme.

To query the new post-type you can use the `cpt` method of the **nuxtpress** `$wp` object.
...
The taxonomy definition is copied into the `taxonomies` folder of the nuxtpress
theme and loaded automatically by the theme.

## Links
<a name="links"/>

[NUXTPRESS Headless WordPress API Extensions](https://wordpress.org/plugins/wuxt-headless-wp-api-extensions/): Plugin which includes all our API extensions.
